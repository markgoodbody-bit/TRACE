# TRACE // v0.3.0 SPINE CANDIDATE v0.5
...
where `V` contains load-bearing stages/events and `E_prec` required precedence.
...
Correction timing needs an explicit target boundary, comparable clocks and a supported process bound; apparent parallelism does not shorten the window unless it is feasible.
...
This spine remains intentionally incomplete relative to v0.2.7. A full v0.3 candidate must account for donor vocabulary, serialization, access/custody algebra, nested-boundary detail, richer coupling/refusability, trajectory correspondence, route executability, burden/record detail, recursive zoom/merge, operator/checker contract, worked transfers and misuse coverage before any replacement claim.
