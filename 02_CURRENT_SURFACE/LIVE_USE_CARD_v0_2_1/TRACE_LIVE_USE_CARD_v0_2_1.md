# TRACE Live Use Card v0.2.1

Status: front-door use instrument / live reasoning surface / micro-patch after Kimi hidden-data failures / not validation / not proof / not governance certification.
...
Patch basis from v0.2: Kimi broke the live-use implementation twice by making the visible hazard frame misleading. This patch adds stronger checks for anomaly fusion, distributed knowledge carriers, and carrier fit before escalation.
...
#### 9.5 Split anomalies before fusing them

Plain question: are multiple signs being collapsed into one story too quickly?

```trace
anomaly_split :=
  list_each_anomaly_separately
  + timestamp_each_anomaly
  + locate_each_anomaly
  + identify_witness_or_carrier_for_each
  + test_same_cause_vs_parallel_causes
```

Minimum practice: write each anomaly as its own line. Do not assume one cause merely because the facts are near each other in time or space.

```trace
example_anomaly_lines :=
  smell_at_2300 := location? duration? witness? log?
  powder_found_0730 := location? container? provenance? witness?
  nausea := exposure? alternative_cause? witness?
  warmth := HVAC? sensor? room? trend?
```

Failure mode: visible hazard capture. A respectable response can carefully solve the wrong fused story.

#### 9.6 Distributed knowledge carrier sweep

Plain question: whose record, log, desk, inbox, keys, partner, rota entry, manual, or last action can still speak even if the person cannot?

```trace
distributed_knowledge_carriers :=
  person
  + notes
  + logbook
  + desk
  ...
  + keys
  + emergency_contact
  + CCTV_or_access_record
  + last_known_action
  + specialist_manual
```

Do not stop at the most obvious absent person. Under hidden-data pressure, the decisive carrier may be a guard log, former employee note, partner message, site binder, maintenance record, or manual.

Failure mode: treating absence as silence.

#### 9.7 Carrier fit before escalation

Plain question: does the specialist or authority you are calling solve the source, or only the visible symptom?
...
carrier_fit_before_escalation :=
  ...
  anomalies_unexplained := ?
  cost_or_delay_created := ?
  reversible_or_locking := ?
```

Use escalation, but do not let escalation become premature closure. A hazmat carrier may solve powder identification but miss HVAC failure. A refrigeration carrier may fix a sensor but miss preservation packaging. A legal carrier may freeze action but miss a physical deadline.

Failure mode: wrong carrier escalation.
...
success_if :=
  ...
  + anomalies_are_not_fused_too_fast
  + carrier_fit_is_tested
...
failure_if :=
  ...
  OR visible_hazard_frame_captures_response
  OR wrong_carrier_solves_visible_symptom_only
```
